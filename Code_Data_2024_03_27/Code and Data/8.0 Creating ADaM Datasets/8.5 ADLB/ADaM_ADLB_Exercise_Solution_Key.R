...
tmp1$AWLO[[2]] <- condition(x == "Week 4", "23") # Override incorrect spec value
tmp1$AWLO[[length(tmp1$AWLO) + 1]] <- condition(TRUE, NA)
lb_fmts$AWLO <- tmp1$AWLO

tmp1$AWHI[[2]] <- condition(x == "Week 4", "36") # Override incorrect spec value
...
dat_lb <- lb %>% select(USUBJID, LBSEQ, LBTESTCD, LBTEST, LBCAT, LBORRES,
  ...
  filter(fapply(LBTESTCD, lb_fmts$LBTESTCDS) == TRUE, LBSTRESC != "",
