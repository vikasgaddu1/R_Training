...
SDTM_path <- "_data/abc/SDTM/"
ADaM_path <- "_data/abc/ADaM/"

# Prepare Formats ---------------------------------------------------------
sep("Load format catalog")
fc_adam <- read.fcat("_data/abc/adam_fmts.fcat") %>% 
  put()

# Import Data ------------------------------------------------------------

adam_specs_path <- "./_specs/ADaM_Specs_Clean.xlsx"
...
ae <- readRDS(file.path(SDTM_path, "AE.rds")) %>% 
  select(USUBJID, AETERM, AEOUT, AESTDTC) %>% 
  filter(AETERM != "" & !is.na(AETERM) & AETERM != " " & AETERM != ".") %>% 
  mutate(FATAL = case_when(AEOUT == "FATAL" ~ 1,
                           TRUE ~ 0),
         DTHDT = case_when(AEOUT == "FATAL" ~ as_date(AESTDTC),
                           TRUE ~ as_date(NA))) %>% 
  group_by(USUBJID) %>% 
  summarize(aecount = as.numeric(n()),
            numfatals = sum(FATAL),
            DTHDT     = max(DTHDT, na.rm=FALSE)) %>% 
  ungroup() %>% 
  transmute(USUBJID  = USUBJID,
            DTHFL    = case_when(numfatals == 0 ~ "N",
                                 TRUE ~ "Y"),
            AEREPORT = case_when(aecount == 0 ~ "N",
                                 TRUE ~ "Y"),
            DTHDT    = DTHDT)
...
adsl_final <-
  ...
  mutate(RANDDT     = as_date(VISIT1),
         ...
         PSORTRTR   = case_when(RESUME1 == 1 | RESUME2 == 1 | 
                                RESUME3 == 1    ~ "Y",
                                TRUE ~ "N"),
         FUPVISIT   = case_when(!is.na(VISIT16) ~ "Y",
                                TRUE ~ "N")) %>% 
  left_join(ae, by = "USUBJID") %>% 
  mutate(DTHFL     = case_when(!is.na(DTHFL)  ~ DTHFL,
                               TRUE ~ "N"),
         AEREPORT  = case_when(!is.na(AEREPORT)  ~ AEREPORT,
                           TRUE ~ "N")) %>% 
  select(STUDYID,USUBJID,SUBJID,SITEID,AGE,AGEU,AGEGR1,SEX,
         RACE,RACEN,ETHNIC,ETHNICN,COUNTRY,ARM,ACTARM,
...
saveRDS(adsl_final, '_data/abc/ADaM/adsl.rds')

# Compare to Reference Dataset --------------------------------------------

qc_adslf <- "_data/abc/ADaM/ADSL.csv"
qc_adsl <-
  read_csv(qc_adslf,
