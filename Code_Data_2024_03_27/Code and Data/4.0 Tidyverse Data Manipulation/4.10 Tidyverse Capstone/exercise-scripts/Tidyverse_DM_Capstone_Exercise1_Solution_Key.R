...
# Load Data
data_demo <-
  read_sas("data/dm.sas7bdat")

data_demo %>% distinct(RACE)

data_demo$racef <- factor(data_demo$RACE,
...
complete_summary <-
  ...
  rename(`Arm A` = ARM.A, `Arm B` = ARM.B, `Arm C` = ARM.C, `Arm D` = ARM.D,
         Variable = var, Measure = label) %>%
  mutate(Variable = paste0(substring(Variable,1,1), substring(tolower(Variable), 2))) %>%
  print()
