...
```{r}
# Create a Vector sex <- c("M", "F", "F", "M")  
# Create a Mapping Vector sex_decode <- c(M = "Male", F = "Female")  
# Apply the value mapping to existing vector sex_decode[sex]
##        M        F        F        M  
...
We can subset a list by using two brackets `[[ ]]`, containing either the index of the vector we want to subset, or the name of the vector.
...
In addition to using the double brackets, we can use the `$` to subset a vector followed by the *unquoted* name of the vector we want to subset.

```{r}
b_list$letters ## [1] "a" "b" "c"
```
...
```{r}
subset(df, sex == "F", select = c("names", "sex")) 
subset(df, age <=  40, select = c("names", "sex"))
```
