...
adsl <-
  bind_rows(read_sas(file.path(dir_data, 'abc_adam_adsl.sas7bdat')), 
            read_sas(file.path(dir_data, 'bbc_adam_adsl.sas7bdat'))) %>% 
  select(USUBJID, SEX, AGEGR1, AGE, ARM) %>% 
  filter(ARM != "SCREEN FAILURE") %>% put()

put("Combine ADPSGA for studies ABC and BBC")
adpsga <-
  bind_rows(read_sas(file.path(dir_data, 'abc_adam_adpsga.sas7bdat')),
            read_sas(file.path(dir_data, 'bbc_adam_adpsga.sas7bdat'))) %>%
  filter(   PARAMCD =="PSGA" &
              TRTA != "" & !is.na(AVISITN)) %>%
...
arm_pop <- adpsga %>% select(USUBJID, TRTA) %>% distinct() %>% 
  count(TRTA) %>% deframe() %>% put()

# These counts are not matching counts from adpsga
...
tbl1 <- create_table(count_block, width = 9) %>% 
  ...
  titles("Table 5.0", "Analysis of Time to Initial PSGA Success* in Weeks", 
         "Safety Population")
...
plt <- create_plot(surv_gg, 3.5, 9) %>% 
  titles("Figure 5.0", "Kaplan-Meier Plot for Time to Initial PSGA Success (PSGA <= 1)",
...
rpt <- create_report("output/Surv_Table.rtf", output_type = "RTF", missing = "-") %>% 
  set_margins(top = 1, bottom = 1) %>% 
  page_header("Sponsor: Anova", "Study: ABC/BBC") %>% 
  ...
  footnotes("Program: Surv_Table.R",
            ...
            "Note: A subject who is not cured by the end of 12 weeks or is lost",
