...
plt1 <- create_plot(figure_1, 5, 9) %>% 
  titles("Figure 2.0", "Subject counts by ARM") %>% 
  footnotes("Note: Need at least 22 subjects to be a valid trial")

plt2 <- create_plot(figure_2, 5, 9) %>% 
  titles("Figure 3.0", "Outliers of Pulse and Temperature (°C)") %>% 
  footnotes("Note: Outliers are marked by a red subject # to the left of the data point")

rpt <- create_report("output/VS_Figures.rtf", output_type = "RTF") %>% 
  page_header("Client: Anova", "Study: ABC") %>% 
  add_content(plt1) %>% 
  add_content(plt2) %>% 
  footnotes("Program: VS_Figures.R") %>% 
