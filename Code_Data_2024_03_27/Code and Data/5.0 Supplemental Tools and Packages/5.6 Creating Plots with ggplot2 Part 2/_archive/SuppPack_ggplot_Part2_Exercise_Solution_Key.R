...
# Exercise Step 5.c
# Now adding a facet variable of AGEGR1
ggplot(plotdata, aes(x = ARM)) +
  geom_bar(fill = 'black', color = 'black') +
  facet_grid(cols = vars(AGEGR1))

# Exercise Step 5.d
# Now switch to a grid of bar charts by introducing a row variable of SEX.
...
# Exercise Step 5.g
# Now add a pink shaded area to the plot that is 2 standard deviations wide from the mean of TEMP and covers the
...
# Exercise Step 6.b
# Comparing Group Means of BMI by ARM.
ggplot(plotdata_summary_BMIbyARM, aes(x = ARM, y = avg)) +
  geom_point(size = 3) +
