...
ui <- shiny::fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(width=2,
      ...
      selectInput(
        inputId  = "select_data",
        label    = "Select an Input Data Frame",
        choices  = fs::dir_ls(data_path, glob = "*.rds"),
        selected = NULL),
      actionButton(
        inputId = "import_data",
        label   = "1. Read Data Frame"),
    ...
    mainPanel(
      ...
      ## Variable Summary ----
      tabsetPanel(
        tabPanel("Table", DT::dataTableOutput("summary_output"))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  df_import <-
    eventReactive(input$import_data, {
      readRDS(input$select_data)
    })

  ## Import Message ----
  output$import_message <- renderPrint(
    str_glue("You read <b>{input$select_data}</b>,
  ...
  observeEvent(input$select_vars, {
    updateActionButton(
      inputId = "use_vars",
      label   = "3. Click to Get Tabular Listing"
    )
  })
  ...
  df_variable_subset <-
    ...
    output$summary_output <- DT::renderDataTable(df_variable_subset())
